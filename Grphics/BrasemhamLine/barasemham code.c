#include <stdio.h>
 ...
 /*  calculates the position for minute and second needle. 360 to 90 degree is equivalent to 45 to 0 th minute in clock.  So, store the points in reverse direction(45 to 0) Once 0 is reached, we need to store points corresponds to 84th degree to 0 degree in reverse direction(59 to 46). So, totally we will get 60 points which corresponds to 60 minute or second position in clock.  */
 ...
 /* find the points at 0, 30, 60,.., 360 degrees on the given circle.  x value correponds to radius * cos(angle) and y value corresponds to radius * sin(angle).  Numbers in the clock are written using the above manipulated x and y values.  And the hour needle movement is based on this */
